mod day17 {
    ...
    pub fn part1(vec: Vec<String>) -> i32 {
        ...
        vec.iter().enumerate().for_each(|(y, line)| {
            line.chars().enumerate().for_each(|(x, c)| {
                ...
                if v > 0 {
                    grid.insert((x as i32, y as i32, 0, 0), v);
                }
            });
        });
        println!("{:?}", grid);
    ...
    fn turn_one(grid: &HashMap<Position, i32>) -> HashMap<Position, i32> {
        ...
        let (x_r, y_r, z_r, w_r) = get_grid_range(&grid);
        for w in w_r.0..w_r.1 + 3 {
            for z in z_r.0..z_r.1 + 3 {
                for y in y_r.0..y_r.1 + 3 {
                    for x in x_r.0..x_r.1 + 3 {
                        let pos = (x as i32 - 1, y as i32 - 1, z as i32 - 1, w as i32 - 1);
            ...
            for j in 0..3 {
                for k in 0..3 {
                    for n in 0..3 {
                        ...
                        let z = pos.2 + k;
                        let w = pos.3 + n;
                        let new_pos = (x as i32 - 1, y as i32 - 1, z as i32 - 1, w as i32 - 1);
                        if &new_pos != pos {
                            list.push(new_pos);
                        }
                    }
    ...
    fn print_grid(grid: &HashMap<Position, i32>) {
        let (x_r, y_r, z_r, w_r) = get_grid_range(&grid);
        for w in w_r.0..w_r.1 + 1 {
            for z in z_r.0..z_r.1 + 1 {
                println!("z={:?}, w={}", z, w);
                for y in y_r.0..y_r.1 + 1 {
                    let mut s = String::from("");
                    for x in x_r.0..x_r.1 + 1 {
                        if let Some(_) = grid.get(&(x, y, z, w)) {
                            s.push('#');
                        } else {
                            s.push('.');
                        }
